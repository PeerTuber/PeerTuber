...
abstract class CommentRemoteDataSource {
  ...
  Future<List<CommentModel>> getCommentsByUrl({
    required String videoUrl,
    required int videoId,
  });
  Future<List<CommentReplyModel>> getReplies(
      {required int videoId, required int threadId});
  Future<List<CommentReplyModel>> getRepliesByUrl(
      {required String videoUrl, required int videoId, required int threadId});
}
...
class CommentRemoteDataSourceImpl implements CommentRemoteDataSource {
  ...
  Future<List<CommentModel>> getCommentsByUrl({
    ...
    final url = '${videoUri.origin}/api/v1/videos/$videoId/comment-threads';
    final uri = Uri.parse(url);
    late http.Response response;
